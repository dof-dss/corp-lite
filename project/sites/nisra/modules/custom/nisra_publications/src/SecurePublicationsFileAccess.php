...
class SecurePublicationsFileAccess {
  ...
  public function canAccessSecureFileAttachment(string $file_uri) {
    $sql = "SELECT DISTINCT
      ...
      JOIN node__field_secure_attachment nfpsf ON nfpsf.field_secure_attachment_target_id = mfd.mid
      JOIN node_field_data nfd ON nfd.nid = nfpsf.entity_id
      WHERE nfd.type = 'secure_market_statistics'
      AND fm.uri = :file_uri";
    ...
    if ($node->isPublished() && $node->bundle() === 'secure_market_statistics') {
      return TRUE;
    }
    else {
      return FALSE;
